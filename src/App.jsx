...
function parseHealthXML(xml) {
  ...
  // 步數（逐行掃描，避免大檔案 regex 卡死）
  const lines = xml.split('\n');
  for (const line of lines) {
    if (line.indexOf('HKQuantityTypeIdentifierStepCount') === -1) continue;
    const dateM = line.match(/startDate="(\d{4}-\d{2}-\d{2})/);
    const valM = line.match(/value="([\d.]+)"/);
    if (!dateM || !valM) continue;
    const date = dateM[1], val = parseInt(valM[1]);
    if (!results[date]) results[date] = {};
    results[date].steps = (results[date].steps || 0) + val;
  }
  return results;
}
function ImportView({ logs, onImport, onNav, showToast }) {
  ...
  const doImport=()=>{
    ...
    Object.entries(parsed).forEach(([date,vals])=>{
      ...
      if(vals.steps && vals.steps > 0) {
        const stepBurn = Math.round(vals.steps * 0.04);
        const existingIdx = (newLogs[date].workouts||[]).findIndex(w=>w.name==="步行（健康 App）");
        if(existingIdx >= 0) {
          newLogs[date].workouts[existingIdx] = { name:"步行（健康 App）", steps:vals.steps, burn:stepBurn };
        } else {
          newLogs[date].workouts = [...(newLogs[date].workouts||[]), { name:"步行（健康 App）", steps:vals.steps, burn:stepBurn }];
        }
        newLogs[date].totalBurn = (newLogs[date].workouts||[]).reduce((s,w)=>s+w.burn,0);
        newLogs[date].steps = vals.steps;
      }
    });
    onImport(newLogs);
  ...
  return (
    <div style={s.main}>
      ...
      {preview&&(<div style={s.goalCard}>
        ...
        {[["筆數",`${preview.count} 天`],["時間範圍",`${preview.from} ～ ${preview.to}`],["最新體重",preview.sample?.weight?`${preview.sample.weight} kg`:"無"],["最新體脂",preview.sample?.bodyfat?`${preview.sample.bodyfat}%`:"無"],["最新 BMI",preview.sample?.bmi?`${preview.sample.bmi}`:"無"],["最新肌肉量",preview.sample?.leanMass?`${preview.sample.leanMass.toFixed(1)} kg`:"無"],["最新步數",preview.sample?.steps?`${preview.sample.steps.toLocaleString()} 步 · 約 ${Math.round(preview.sample.steps*0.04)} kcal`:"無"]].map(([k,v])=>(
          <div key={k} style={s.goalRow}><span style={s.goalKey}>{k}</span><span style={s.goalVal}>{v}</span></div>
        ))}
