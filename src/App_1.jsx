...
function parseHealthXML(xml) {
  ...
  // 步數（累加當天所有筆）
  const stepReg = /<Record[^>]*type="HKQuantityTypeIdentifierStepCount"[^>]*startDate="(\d{4}-\d{2}-\d{2})[^"]*"[^>]*value="([\d.]+)"[^>]*\/?>/g;
  let sm;
  while ((sm = stepReg.exec(xml)) !== null) {
    const date = sm[1], val = sm[2];
    if (!results[date]) results[date] = {};
    results[date].steps = (results[date].steps || 0) + parseInt(val);
  }
  return results;
}
